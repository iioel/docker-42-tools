...
function start_kasmvnc (){
	...
	echo "DisableBasicAuth=1" >> /home/kasm-user/.vnc/config
	vncserver $DISPLAY -depth $VNC_COL_DEPTH -geometry $VNC_RESOLUTION -websocketPort $NO_VNC_PORT -DisableBasicAuth -httpd ${KASM_VNC_PATH}/www -FrameRate=$MAX_FRAME_RATE -interface 0.0.0.0 $VNCOPTIONS $KASM_SVC_SEND_CUT_TEXT $KASM_SVC_ACCEPT_CUT_TEXT
...
function start_audio_in (){
	if [[ ${KASM_SVC_AUDIO_INPUT:-1} == 1 ]]; then
		...
		$STARTUPDIR/audio_input/kasm_audio_input_server --auth-token "kasm_user:$VNC_PW" &
...
function start_upload (){
	if [[ ${KASM_SVC_UPLOADS:-1} == 1 ]]; then
		...
		./kasm_upload_server --auth-token "kasm_user:$VNC_PW" &
